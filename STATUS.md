...
在 v1（17 页高保真还原）的基础上完成 **v2 迭代**：产品重心收敛为「岗位申请包」，页面收敛到 8 个，整条主流程真实可操作并形成反哺闭环——**导入旧简历 → 访谈补全 → 职业证据 → 导入岗位 → 岗位专属简历 → 面试 QA → 模拟面试 → 真实录音复盘 → 复盘结果经确认后更新下一版 QA / 简历 / 证据**。v2.1 接入两项真实能力：**Boss 直聘爬虫产物批量导入岗位**、**本地 Chrome 真实导出简历 PDF**（Telos 同构）。v2.2 完成**真实 AI 接入**（设置页填用户自己的 API URL + Key，双协议透传，平台不托管）与**全量本机持久化**（IndexedDB + 备份导出/导入迁移）；未配置 Key 时保持 mock 演示模式。v2.3 为模拟面试 / AI 访谈加入**浏览器实时语音输入**（免打字，识别结果确认后发送）。
...
## Agent 能力规划（已评估，待用户排期，暂不实现）

结论：**不做全自动求职 Agent**（产品护城河恰恰是证据确认、逐条采纳这套人工关卡；自动投递已明确排除）。但三个位置的「带确认关卡的 Agent」是真提升，按优先级：

1. **批量岗位分析 Agent**：Boss 导入 N 个岗位后排队跑「JD 拆解→证据匹配→覆盖度排序→每岗缺口清单」，回答“这批岗位先准备哪个”。只做分析不产内容，零原则冲突；可先用现有 `ask()` 排队实现够用版，不必等 Agent 框架。
2. **带工具的访谈/模拟面试官**：给面试官两个工具——`查证据库`（追问前核对“这个说法有没有已确认证据”，没有就深挖）、`起草证据卡`（访谈中实时沉淀草稿，结束后用户确认入库）。确认关卡保留。
3. **「一键备齐申请包」编排**：分析 JD→生成简历→（**停：用户逐条确认简历建议**）→生成 QA 的链式执行。

技术前提：`ask()` 需加工具调用循环；Anthropic 与 OpenAI 的 tool-call 格式不同，**Agent 功能先只支持 Anthropic 原生协议**，OpenAI 兼容档继续用普通生成。Agent 烧用户自己的 Key，界面必须实时展示每一步动作（透明 + 刹车）。
